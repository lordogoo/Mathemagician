...
class webSocketStateAuthenticate extends webSocketState{
	processMessage(message) {
		...
		let querySession = "Select session_id,data,expires from sessions where data->'$.user' = ? and data->'$.sessionID' = ? ";
		let query = mysql.format(querySession,[userName,sessionId]);
...
class webSocketStateSwapEquations extends webSocketState{
	processStart(){
		let queryEquations = "select (Select equation from equation where id=?) as playerequation,(Select equation from equation where id=?) as opponentequation;";
		let query = mysql.format(queryEquations,[this.socket.MathemagicianMetaData.equation,this.socket.MathemagicianOpponent.MathemagicianMetaData.equation]);
		this.dbConnection.query(query , (err, result) => {
			if(err){
				console.log(err);
				//invalidate connection
				...
				let equationData = {}; 
				equationData.playerEquation = result[0].playerequation;
				equationData.opponentEquation = result[0].opponentequation;
...
class webSocketStateEndMatch extends webSocketState{
	processStart(){
		...
		let queryMatchResult = "insert into history(playerName,opponentName,playerEquation,opponentEquation,gameMode,result,whentime) values(?,?,?,?,?,?,?)";
		let query = mysql.format(queryMatchResult,[playerName,opponentName,playerEquation,opponentEquation,gameMode,result,whentime]);
		this.dbConnection.query(query, (err, result) => {
			if(err){
				console.log(err);
			}else{
